...
def readworkspace( wspname, upsize ):
    ...
    # etc) should be last. The algorythm below does this exactly but using regex
    ...
    for line in f:
        ...
        if searchmode =='display':
            ...
            m = re.search('(display name="([a-zA-Z\ 0-9]+)".+dim="([0-9\,]+)".+extent="([0-9\.\-\,]+)")',line)    # NEED TO HANDLE more displays...!
            if m:
                display=m.group(2)
                extentall=m.group(4)
                ...
                searchmode='layer'
        if searchmode =='layer':
            m = re.search('(layer type="([a-zA-Z]+)".+checked="1"\ opacity="([0-9\.]+)"|overlay name="([a-zA-Z\.]+)")',line)
            if m:
                if m.group(2):
                    layer = m.group(2)
            ...
            if m:
                ...
                if m:
                    ...
                    upsized = upsizeifnecessary(task,lastparam,htmldecoded,upsize)
                    ...
                    if m:
                        if startnew:
                            if m.group(1):
                                layers.insert(0,(opacity,task+flag+parameters,paramdic,flagdic))
                            else:
                                layers.append((opacity,task+flag+parameters,paramdic,flagdic))
                            grass.verbose(_(opacity +' >> '+str(display)+" >> "+ task+flag+parameters +" pd:"+str(paramdic)+" fd:"+str(flagdic)))
                        else:
                            if m.group(1):
                                layers.insert(0,('1',task+flag+parameters,paramdic,flagdic))
                            else:
                                layers.append(('1',task+flag+parameters,paramdic,flagdic))
                            grass.verbose(_('1 >> '+str(display)+" >> "+ task+flag+parameters +" pd:"+str(paramdic)+" fd:"+str(flagdic)))
                        parameters = ''
                        paramdic = {}
                        flag = ''
                        flagdic = {}
...
def getfontbypattern(kindpattern):
    ...
    split=s.splitlines()
    for l in split:
        m = re.search('[\:\ ]+',l,re.IGNORECASE)      #check if it has : or space.
        if not m:
...
def main():
    ...
    savedregionname = "tmp.%s.%d" % (os.path.basename(sys.argv[0]), os.getpid())
    ...
    for key in displays:
        ...
        extents=layers[0]
        grass.verbose("m.printws: EXTENTS from workspace:"+str(extents))   #was debug message
        del layers[0]
        ...
        if len(options['region'])>0:
            ...
            grass.run_command("g.region", "", w=extents[0], s=extents[1], e=extents[2], n=extents[3])
            regionmode='window'
        ...
        if len(options['titlefont'])>1:
            ...
            if isAsterisk>0:
                titlefont=getfontbypattern(options['titlefont'].replace('*',''))
            else:
        ...
        if len(options['maintitlesize'])>1:
            maintitlesize=converttommfrom(float(options['maintitlesize']),options['layunits'])
        else:
            maintitlesize=10.0

        if len(options['subtitlesize'])>1:
            subtitlesize=converttommfrom(float(options['subtitlesize']),options['layunits'])
        else:
            subtitlesize=7.0
        ...
        maxframe = str(mxfd['t'])+','+str(mxfd['b'])+','+str(mxfd['l'])+','+str(mxfd['r'])
        ...
        if hregionratio>hmapratio:
            grass.verbose(_("printws: Map area height correction / "+str(hregionratio)))
            mapsizes['h']=mapsizes['w']/hregionratio
        elif hregionratio<hmapratio:
            grass.verbose(_("printws: Map area width correction * "+str(hregionratio)))
        ...
        mapframe = str(mpfd['t'])+','+str(mpfd['b'])+','+str(mpfd['l'])+','+str(mpfd['r'])
        ...
        imcommand='convert  -limit memory 720000000 -limit map 720000000 -units PixelsPerInch -density '+str(int(dpioption))+' '
        ...
        for lay in layers:
            ...
            if lay[0]=='1':
                if lastopacity <> '1':
                    LASTFILE = os.path.join(TMPDIR, str(os.getpid())+'_DIS_'+str(displaycounter)+ '_GEN_' + str(LAYERCOUNT) + '.'+TMPFORMAT)
                ...
                render(lay[1],lay[2],lay[3])
                imcommand=imcommand+' \( '+LASTFILE+' -channel a -evaluate multiply '+lay[0]+' +channel \)'

        # setting resolution back to pre-script state since map rendering is finished
        ...
        params= ' -compress jpeg -quality 92 '+pagedata['parameters']+' -units PixelsPerInch -density '+str(int(dpioption))+' '

        imcommand=imcommand+' -layers flatten '+params+'"'+output+displaypart+'.'+extension+'"'
    ...
    grass.run_command('g.remove', flags="f", type="region", pattern=savedregionname)
